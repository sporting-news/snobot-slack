...
#
# Commands:
#   hubot redmine view me <issue-id> - Show the issue status
#   hubot redmine show (my|user's) issues - Show your issues or another user's issues
#   hubot redmine assign <issue-id> to <user-first-name> ["notes"] - Assign the
#       issue to the user (searches login or firstname)
#   hubot redmine update <issue-id> with "<note>" - Adds a note to the issue
#   hubot redmine add <hours> hours to <issue-id> ["comments"] - Adds hours to
#       the issue with the optional comments
#   hubot redmine link me <issue-id> - Returns a link to the redmine issue
#   hubot redmine set <issue-id> to <int>% ["comments"] - Updates an issue and
#       sets the percent done
#
# Notes:
#   <issue-id> can be formatted in the following ways:
#   1234, #1234, issue 1234, issue #1234
#
# Author:
#   robhurring and jcopp
...
module.exports = (robot) ->
  ...
  # Robot redmine link me <issue>
  robot.respond /redmine link me (?:issue )?(?:#)?(\d+)/i, (msg) ->
    id = msg.match[1]
    msg.reply "#{redmine.url}/issues/#{id}"

  # Robot redmine set <issue> to <percent>% ["comments"]
  robot.respond /redmine set (?:issue )?(?:#)?(\d+) to (\d{1,3})%?(?: "?([^"]+)"?)?/i, (msg) ->
  ...
  # Robot redmine add <hours> hours to <issue_id> ["comments for the time tracking"]
  robot.respond /redmine add (\d{1,2}) hours? to (?:issue )?(?:#)?(\d+)(?: "?([^"]+)"?)?/i, (msg) ->
  ...
  # Robot redmine show <my|user's> [redmine] issues
  robot.respond /redmine show (?:my|(\w+\'s)) (?:redmine )?issues/i, (msg) ->
    userMode = true
    firstName =
      if msg.match[1]?
        userMode = false
  ...
  # Robot redmine update <issue> with "<note>"
  robot.respond /redmine update (?:issue )?(?:#)?(\d+)(?:\s*with\s*)?(?:[-:,])? (?:"?([^"]+)"?)/i, (msg) ->
  ...
  # Robot redmine add issue to "<project>" [traker <id>] with "<subject>"
  robot.respond /redmine add (?:issue )?(?:\s*to\s*)?(?:"?([^" ]+)"? )(?:tracker\s)?(\d+)?(?:\s*with\s*)("?([^"]+)"?)/i, (msg) ->
  ...
  # Robot redmine assign <issue> to <user> ["note to add with the assignment]
  robot.respond /redmine assign (?:issue )?(?:#)?(\d+) to (\w+)(?: "?([^"]+)"?)?/i, (msg) ->
  ...
  # Robot redmine view me <issue>
  robot.respond /redmine view(?: me)? (?:issue )?(?:#)?(\d+)/i, (msg) ->
    id = msg.match[1]
